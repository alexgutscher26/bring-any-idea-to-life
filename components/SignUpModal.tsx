...
import { useSession, signUp } from '@/services/authClient'
        ...
        <div className="p-6">
          ...
          <form onSubmit={submit} className="space-y-3">
            <div>
              ...
              <input type="text" value={name} onChange={e=>setName(e.target.value)} required className="w-full mt-1 px-3 py-2 rounded-lg bg-zinc-900 border border-zinc-800 text-white outline-none focus:border-zinc-600" />
            </div>
            <div>
            ...
            <button type="submit" disabled={loading} className="w-full py-2.5 rounded-lg bg-white text-black font-bold hover:bg-blue-50 disabled:opacity-50">{loading ? 'Creating...' : 'Sign Up'}</button>
          </form>
        </div>
      </div>
    </div>
